...
foreach ($node in $env.Nodes) {
    ...
    $data = Invoke-ProxmoxApiGET -Endpoint "nodes/$serverName/qemu" -Token_Name $secrets.Token_Name -API_Token $secrets.API_Token -FQDNorIP $FQDNorIP
...
#$data = .\Task\Get-qemuVM.ps1
#$data | select Name, ServerName, status